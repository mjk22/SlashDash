...
Value mnbudget(const Array& params, bool fHelp)
{
    ...
    if (fHelp ||
        ...
        throw runtime_error(
            ...
            "  vote-many          - Vote on a SlashDash initiative\n"
            "  vote-alias         - Vote on a SlashDash initiative\n"
            "  vote               - Vote on a SlashDash initiative/budget\n"
...
Value preparebudget(const Array& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 6)
        throw runtime_error(
            "preparebudget \"proposal-name\" \"url\" payment-count block-start \"slashdash-address\" monthy-payment\n"
            ...
            "5. \"slashdash-address\":   (string, required) SDASH address to send payments to\n"
    ...
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid SlashDash address");

    // Parse SlashDash address
...
Value submitbudget(const Array& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 7)
        throw runtime_error(
            "submitbudget \"proposal-name\" \"url\" payment-count block-start \"slashdash-address\" monthy-payment \"fee-tx\"\n"
...
Value getbudgetprojection(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "    \"PaymentAddress\": \"xxxx\",     (string) SDASH address of payment\n"
