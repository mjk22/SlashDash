...
def initialize_datadir(dirname, n):
    ...
    if not os.path.isdir(datadir):
        os.makedirs(datadir)
    with open(os.path.join(datadir, "slashdash.conf"), 'w') as f:
        f.write("regtest=1\n");
        f.write("rpcuser=rt\n");
...
def initialize_chain(test_dir):
    ...
    4 wallets.
    divid and slashdash-cli must be in search path.
    """
    ...
    if not os.path.isdir(os.path.join("cache", "node0")):
        ...
        for i in range(4):
            ...
            subprocess.check_call([ os.getenv("BITCOINCLI", "slashdash-cli"), "-datadir="+datadir,
    ...
    for i in range(4):
        ...
        initialize_datadir(test_dir, i) # Overwrite port/rpcport in slashdash.conf
